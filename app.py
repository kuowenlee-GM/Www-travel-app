...
with st.sidebar:
    ...
    with st.expander("➕ 新增樂樂小物"):
        new_dest = st.selectbox("目的地", ["國內", "國外", "季節需求"])
        new_scene = st.text_input("場景/季節名稱")
...
if season_select != "無":
    season_items = st.session_state.ITEM_DATABASE["季節需求"].get(season_select, [])
    for idx, s_item in enumerate(season_items):
